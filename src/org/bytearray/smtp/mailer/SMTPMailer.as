...
package org.bytearray.smtp.mailer {
	...
	public class SMTPMailer extends Socket implements ISMTPMailer {
		...
		*/
		...
		private function evt(response:String, smtpReturn:Number, smtpInfos:SMTPInfos):Boolean {
			...
			if (smtpReturn == SMTPMailer.READY)
			...
			else if (smtpReturn == SMTPMailer.ACTION_OK && ((response.toLowerCase().indexOf("queued") != -1 || response.toLowerCase().indexOf("accepted") != -1 || response.toLowerCase().indexOf("qp") != -1 || response.toLowerCase().indexOf("id=") != -1 || response.toLowerCase().indexOf("2.0.0") != -1) && lastStatus == DATA))
				dispatchEvent(new SMTPEvent(SMTPEvent.MAIL_SENT, smtpInfos));

			else if (smtpReturn == SMTPMailer.ACTION_OK && (response.toLowerCase().indexOf("ok") != -1 || response.toLowerCase().indexOf("\r\n") != -1)) {
				//do nothing
			} else if (smtpReturn == SMTPMailer.ACTION_OK) {
				//do nothing
			}
				...
				rv = false;
			} else if (smtpReturn == SMTPMailer.AUTH_DETAILS)
				sendAuth(response);
